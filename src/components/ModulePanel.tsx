...
export default function ModulePanel() {
  ...
  const { 
    files, setFiles, folders, setFolders, openFolders, setOpenFolders,
    view, setView, activeFileId, setActiveFileId, searchQuery, setSearchQuery,
    showUpload, setShowUpload, pendingUpload, setPendingUpload, 
    pendingUploadQueue, setPendingUploadQueue, collapsed, setCollapsed,
    selectMode, setSelectMode, selectedIds, setSelectedIds,
    menuFileId, setMenuFileId, moveFileId, setMoveFileId,
    folderMenuId, setFolderMenuId, editingFolder, setEditingFolder,
    addingFolder, setAddingFolder, dragOver, setDragOver,
    inspectorMenuOpen, setInspectorMenuOpen, renamingFileId, setRenamingFileId,
    labelEditOpen, setLabelEditOpen, showInfo, setShowInfo
  } = useModule();
  ...
  useEffect(() => {
    const handleDocClick = (e: MouseEvent) => {
      ...
      if (!target.closest('.cmp-menu') && !target.closest('.cmp-dot') && !target.closest('.cmp-folder-dot')) {
        if (menuFileId || moveFileId) { setMenuFileId(null); setMoveFileId(null); }
  ...
  const updateFile = (id: number, patch: Partial<ModuleFile>) => {
    setFiles(prev => {
      ...
      if (updated && activeProjectId) DB.references.put({ ...updated, project_id: activeProjectId });
      return next;
    });
  };
  ...
  const removeFile = (id: number) => {
    ...
    if (activeFileId === id) { setView("root"); setActiveFileId(null); }
  ...
  const duplicateFile = (file: ModuleFile) => {
    const copy = { ...file, id: Date.now(), label: `${file.label} COPY`, uuid: crypto.randomUUID() };
    if (activeProjectId) DB.references.put({ ...copy, project_id: activeProjectId });
    setFiles(prev => {
  ...
  const handleFileUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    Promise.all(filesToProcess.map(file => {
      return new Promise<{url: string, file: File}>((resolve) => {
        ...
        reader.onload = (evt) => resolve({ url: evt.target?.result as string, file });
        reader.readAsDataURL(file);
      });
    })).then(uploads => {
      if (replaceMode && activeFileId !== null) {
        const upload = uploads[0];
        updateFile(activeFileId, { url: upload.url, name: upload.file.name, size: Math.round(upload.file.size / 1024) + " KB", uuid: crypto.randomUUID(), visionDesc: "" });
      } else {
        const queue = [...uploads];
  ...
  const handleUploadConfirm = () => {
    ...
    const labelInput = document.getElementById('cmp-upload-label') as HTMLInputElement;
    ...
    if (settings.googleApiKey) {
      import('@/lib/pipeline/vision').then(vision => {
        vision.describe(newFile.url, newFile.label, "subject", settings.googleApiKey)
  ...
  const renderUploadForm = () => {
    ...
    return (
      <div className="cmp-upload-form">
        ...
        <input 
          id="cmp-upload-label" 
          defaultValue={pendingUpload.file.name.replace(/\.[^.]+$/, '').replace(/[_-]+/g, ' ').trim().toUpperCase()} 
    ...
    if (moveFileId === f.id) {
      return (
        <div className="cmp-menu cmp-move-menu">
          ...
          {folders.map(folder => (
            <button key={folder.id} className={f.folder === folder.id ? 'current' : ''} onClick={(e) => { e.stopPropagation(); assignFile(f.id, folder.id); setMoveFileId(null); }}>
              <i style={{background: folder.accent}}></i>
              <span>{folder.name}</span>
              {f.folder === folder.id && <b>NOW</b>}
            </button>
          ))}
          <button onClick={(e) => { e.stopPropagation(); setMoveFileId(null); }}>CANCEL</button>
        </div>
      );
    }
    return (
      <div className="cmp-menu">
        ...
        <button className="primary" onClick={(e) => { e.stopPropagation(); openStudio({ uuid: f.uuid, imgUrl: f.url }); setMenuFileId(null); }}>STUDIO</button>
        <button onClick={(e) => { e.stopPropagation(); setRenamingFileId(f.id); setMenuFileId(null); }}>RENAME</button>
        <button onClick={(e) => { e.stopPropagation(); setMoveFileId(f.id); setMenuFileId(null); }}>MOVE TO...</button>
        <button onClick={(e) => { e.stopPropagation(); duplicateFile(f); setMenuFileId(null); }}>DUPLICATE</button>
        <button className="danger" onClick={(e) => { e.stopPropagation(); removeFile(f.id); setMenuFileId(null); }}>REMOVE</button>
      </div>
    );
  };
        ...
        <div className="cmp-row-main">
          {renaming ? (
            <input 
              ...
              onBlur={(e) => { updateFile(f.id, { label: e.target.value.toUpperCase() || 'UNLABELED' }); setRenamingFileId(null); }}
              onKeyDown={(e) => { if (e.key === 'Enter') e.currentTarget.blur(); if (e.key === 'Escape') setRenamingFileId(null); }}
          ...
          <div className="cmp-row-meta">
            {f.folder === null && <span className="cmp-loose-tag">UNASSIGNED</span>}
            <span>{f.dims}</span>
            {!f.eye && <span>HIDDEN</span>}
            <span className="cmp-mini-strength"><i style={{width: `${f.strength}%`}}></i></span>
            <span>{f.strength}%</span>
          </div>
        </div>
        <button 
          className={`cmp-dot ${menuFileId === f.id || moveFileId === f.id ? 'open' : ''}`} 
          onClick={(e) => { e.stopPropagation(); setMenuFileId(menuFileId === f.id ? null : f.id); setMoveFileId(null); }}
        >&#8943;</button>
        {renderRowMenu(f)}
      </div>
    );
  ...
  const renderFolderForm = (folder: ModuleFolder | null) => {
    const isNew = !folder;
    const activePresetIds = folders.filter(f => f.id !== folder?.id).map(f => f.id);
    const options = MODULE_PRESETS.filter(p => !activePresetIds.includes(p.id));
    
    const lockName = !isNew && options.length === 1 && options[0].id === folder?.id;
    ...
    const handleSave = () => {
      ...
      if (folder) {
        ...
        setFolders([...folders, { id: preset.id, name: preset.name, accent: folderFormAccent }]);
    ...
    return (
      <div className="cmp-folder-form">
        ...
        <div className="cmp-form-actions">
          <button className="primary" onClick={handleSave}>{isNew ? 'CREATE' : 'SAVE'}</button>
          <button onClick={() => { setAddingFolder(false); setEditingFolder(null); }}>CANCEL</button>
        </div>
      </div>
    );
  };
  ...
  const renderFolder = (folder: ModuleFolder) => {
    ...
    const list = files.filter(f => f.folder === folder.id).sort((a,b) => b.modified.localeCompare(a.modified));
    const preview = list[0] ? <span className="cmp-folder-preview">{renderThumb(list[0])}</span> : null;
    const slip = list.length > 1 ? <span className="cmp-folder-slip">+{list.length - 1}</span> : null;
      ...
      >
        <div className={`cmp-folder-head ${open ? 'open' : ''}`} style={{background: open ? folder.accent : ''}}>
          <button className="cmp-folder-toggle" onClick={() => {
            ...
            <span className="cmp-folder-icon">{preview}{slip}</span>
            <span className="cmp-folder-name">{dragOver === folder.id ? `DROP INTO ${folder.name}` : folder.name}</span>
            <span className="cmp-count">{list.length}</span>
          </button>
          <button 
            className={`cmp-folder-dot ${menuOpen ? 'open' : ''}`} 
            onClick={(e) => { e.stopPropagation(); setFolderMenuId(menuOpen ? null : folder.id); }}
          >&#8943;</button>
          
          {menuOpen && (
            <div className="cmp-menu cmp-folder-menu">
              <div className="cmp-menu-title">{folder.name}</div>
              <button className="primary" onClick={(e) => { e.stopPropagation(); setEditingFolder(folder.id); setFolderMenuId(null); }}>EDIT</button>
        ...
        {open && <div className="cmp-folder-body">{list.map(f => renderImageRow(f, false))}</div>}
      </div>
    );
  };
  ...
  const renderRoot = () => {
    ...
    const rootFiles = files.filter(f => f.folder === null).sort((a,b) => b.modified.localeCompare(a.modified));
    const assigned = files.filter(f => f.folder !== null);
    const results = q ? assigned.filter(f => {
      const folder = folders.find(x => x.id === f.folder);
      return f.label.toLowerCase().includes(q) || (folder && folder.name.toLowerCase().includes(q));
    }) : [];

    return (
      <div className="cmp-panel">
        <div className="cmp-header">
          <button className="cmp-header-collapse" onClick={() => setCollapsed(!collapsed)} title="Collapse brief"></button>
          <span>MODULE</span>
        </div>
        <div className="cmp-search">
          <span>&#8981;</span>
          <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="SEARCH" />
          {searchQuery && <button onClick={() => setSearchQuery("")}>&times;</button>}
        </div>
  ...
  const renderInspector = () => {
    ...
    const handleStrengthChange = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      updateFile(f.id, { strength: Math.max(0, Math.min(100, Math.round(((e.clientX - r.left) / r.width) * 100))) });
    };
    ...
    return (
      <div className="cmp-panel">
        <div className="cmp-detail-nav">
          <button onClick={() => { setView('root'); setActiveFileId(null); }}>&lsaquo;</button>
          <span>{folder ? folder.name : 'ROOT'} &rsaquo; <b>{f.label || 'UNLABELED'}</b></span>
          <div className="cmp-inspector-menu-wrap">
            <button className={`cmp-dot ${inspectorMenuOpen ? 'open' : ''}`} onClick={() => setInspectorMenuOpen(!inspectorMenuOpen)}>&#8943;</button>
            {inspectorMenuOpen && (
              <div className="cmp-menu cmp-inspector-menu">
                <div className="cmp-menu-title">{f.label || 'UNLABELED'}</div>
                <button className="primary" onClick={() => { openStudio({ uuid: f.uuid, imgUrl: f.url }); setInspectorMenuOpen(false); }}>STUDIO</button>
                <button onClick={() => { setInspectorMenuOpen(false); fileInputRef.current?.click(); }}>REPLACE</button>
                <button className="danger" onClick={() => { setInspectorMenuOpen(false); removeFile(f.id); }}>REMOVE</button>
              </div>
            )}
          </div>
        </div>
        <div className="cmp-detail-body">
          <div className="cmp-image-card-stack">
            ...
            <div className="cmp-label-card">
              <input 
                ...
                defaultValue={f.label} 
                onBlur={(e) => { updateFile(f.id, { label: e.target.value.toUpperCase() || 'UNLABELED' }); setLabelEditOpen(false); }}
                onKeyDown={(e) => { if (e.key === 'Enter') e.currentTarget.blur(); }}
              />
              <button className="cmp-label-edit" onClick={() => setLabelEditOpen(true)}>EDIT</button>
            </div>
          </div>
          ...
          <div className="cmp-detail-section">
            ...
            <div className="cmp-segments">
              {MODES.map(m => <button key={m} className={f.mode === m ? 'active' : ''} onClick={() => updateFile(f.id, { mode: m })}>{m}</button>)}
            </div>
            <p>{modeHelp(f.mode)}</p>
          </div>
          ...
          <div className="cmp-detail-section">
            <h4>STRENGTH</h4>
            <div className="cmp-strength-head"><span></span><b>{f.strength}%</b></div>
            ...
            <div className="cmp-scale"><span>SUBTLE</span><span>STANDARD</span><span>FORCEFUL</span></div>
          </div>
        ...
        {showInfo && (
          <div className="cmp-info open">
            <span>FILE</span><b>{f.name}</b>
            <span>SIZE</span><b>{f.size}</b>
            <span>DIM</span><b>{f.dims}</b>
          </div>
        )}
        <div className="cmp-status">
          <button onClick={() => setShowInfo(!showInfo)}>INFO {showInfo ? '\u25b2' : '\u25bc'}</button>
          <span>{f.strength}% &middot; {f.mode}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="right-sidebar">
      ...
      <input type="file" id="mp-file-input" accept="image/*" multiple style={{ display: 'none' }} ref={fileInputRef} onChange={handleFileUpload} />
    </div>
  );
}
