...
### 2026-06-04 � Next.js + React Migration (CafeHTML-Next)

**Status:** COMPLETED

**What Was Done:**

Full architectural migration from Vanilla HTML/JS to Next.js (App Router), React, and TypeScript.

**Architecture Changes:**
- **State Management:** `*` globals replaced with React Context providers (`GalleryContext`, `ModuleContext`, `StudioContext`, `SettingsContext`).
- **Componentization:** Extracted monolithic inline HTML into isolated functional components (`src/components/PromptBar.tsx`, `HUD.tsx`, `Studio.tsx`, `ModulePanel.tsx`, etc.).
- **Type Safety:** Migrated all logic files (`src/lib/api.ts` -> `api.ts`, `src/lib/enhancer.ts` -> `enhancer.ts`, etc.) to strict TypeScript. Payload shapes (`GenerationPayload`, `GenerationSettings`) are now strictly typed.
- **Persistence:** IndexedDB wrapper (`src/lib/db.ts`) maintained, but refactored to work harmoniously with React `useEffect` hydration.
- **Styling:** Migrated `src/app/globals.css` into `src/app/globals.css`. Replaced imperative DOM styling with reactive data-driven styles and class toggles.
- **Build Pipeline:** App now builds statically via `next build` using Turbopack. All `react-hooks/exhaustive-deps` and sync-state-in-effect issues resolved.

**Decisions:**
- Kept standard CSS rather than rewriting entirely in Tailwind to preserve exact visual parity.
- Preserved the existing UUID-backed IndexedDB storage mechanism (`DB.images`, `DB.gallery`, etc.) so future features match the legacy data model.
- Kept the parallel `Promise.allSettled` architecture for concurrent image variations via `googleGenerate`.

**Files Touched:**
- Entire `src/` directory created.
- Original HTML/JS logic completely deprecated in favor of React contexts and standard module imports.
...
**`src/lib/api.ts` — Per-variation gallery resolution:**
...
**`src/components/Gallery.tsx` — Error cell methods:**
...
**`src/app/globals.css` — Error cell styles:**
...
**`src/lib/enhancer.ts` — VisionScan pipeline wired in:**
...
**`src/lib/api.ts` — Cleanup + retry tuning:**
...
**Synced to main file:**
- [x] Confirmed synced — `liveRefChips`, `liveUpload`, `refState` all present in `src/app/page.tsx`
...
**Removed — `refVisionCache` (entire layer):**
- `src/lib/enhancer.ts` — removed refVisionCache read (`var desc = null` now) and write block after vision scan
- `src/lib/workspace.ts` — removed `visionCache` from autosave, removed `refVisionCache = ...` on project load
- `src/app/page.tsx` — removed `refVisionCache = {}` init, removed two write calls in ref upload handler

**Removed — `if (X)` guards throughout:**
- `src/lib/workspace.ts` — removed guards on Gallery, refState, renderChips, PromptBuilder, applyModuleState
- `src/lib/api.ts` — removed entry guard for PromptBuilder/CafeSettings, removed CafeDebug ternary (debugEntry always created now), removed PromptEnhancer ternary, removed all 4× `if (debugEntry)` blocks, removed 2× Workspace autosave guards, removed 2× Gallery removeLoading guards, removed payload.settings double-guard
...
**Also fixed:**
- `src/lib/vision.ts` — cache key extended from 32 → 128 chars (all 3 cache functions)
- `src/app/page.tsx` — off-palette colors: `#c3c3c2` → `#c7c7c7` (4 places), `#666666` → `#999997`
- `src/app/page.tsx` — removed 12+ additional `if (X)` guards from chip remove, upload, projects panel, module actions, HUD, upscale handler

**Not changed:**
- SCENE mode structure (refState.SCENE, sequence store) — kept intact, SCENE not built yet
- Modal DOM null checks in settings.js init/renderModal/openModal/closeModal — kept, these are startup guards not X guards
...
**Logic changes:**
- `src/lib/api.ts` — removed `REQUEST_LIMIT` and both generation limit guards. `_activeRequests` kept for button loading state only.
...
**What Was Built:**
- `ModuleContext state` global — live object tracking all SUBJECT, STAGE, STYLE slot states
...
- `prompt-builder.js` rewritten — reads `ModuleContext state` instead of DOM scraping; parses each slot's HTML snapshot in a detached element to read all slots (not just active)
- `compilePrompt()` in `src/lib/api.ts` — assembles prompt text from active visible layers across SUBJECT + STAGE + STYLE, prepended with the user's freeform prompt

**Architecture:**
- `ModuleContext state = { subject: { selected, slots: [{on, html}] }, stage: {...}, style: { selected, slots: [{on, mode, linked, imgUrl, promptText}] } }`
...
**Files Touched:**
- `CafeHTML/src/app/page.tsx` — makeSection signature, syncModuleState, syncStyle, ModuleState init
- `CafeHTML/logic/prompt-builder.js` — full rewrite of collectSection/collectStyle
- `CafeHTML/src/lib/api.ts` — added compilePrompt(), wired into buildPayload and generate()
...
Extracted all ~2400 lines of inline JavaScript from `src/app/page.tsx` into 5 separate `logic/` modules. The primary motivation was reducing context window cost — reading the full HTML file for every edit was expensive and slow. Secondary benefit: each module is now independently editable.

**Files created:**

- **`src/components/PromptBar.tsx`** (~340 lines) — Prompt bar IIFE, reference chips, title bar tabs, cafe menu, projects panel IIFE, mod strip collapse. Exposes `refState`, `renderChips`, `ProjectsPanel`.
- **`src/components/ModulePanel.tsx`** (~745 lines) — Full module panel IIFE: `makeComposeHTML`, `makeClrHTML`, `makeGroupHTML`, `makeSection`, `ModuleState`, `applyModuleState`, file upload handler. Exposes `ModuleContext state`, `applyModuleState`.
- **`src/components/Gallery.tsx`** (~670 lines) — Gallery + HUD IIFE: CELLS array, buildGrid, select mode, HUD viewer, info panel, threedot actions. Exposes `GalleryContext`, `getHudCell`, `closeHUD`.
- **`src/components/SequenceBar.tsx`** (~60 lines) — Sequence bar IIFE: seqSlots, addSeqSlot, getSeqSlots, clearSeqSlots, HUD integration.
- **`src/components/Refine.tsx`** (~585 lines) — Full RefineArea IIFE with pencil, crop, history, ref chips, API call. Exposes `RefineArea`.
...
**Load order:** All 5 new scripts load before the existing 8 logic/ scripts. All cross-module communication is through `*` globals resolved at click time, so no load-order dependencies exist between the 5 extracted files.
...
**Files Touched:**
- `src/app/page.tsx`
- `src/components/PromptBar.tsx` (created)
- `src/components/ModulePanel.tsx` (created)
- `src/components/Gallery.tsx` (created)
- `src/components/SequenceBar.tsx` (created)
- `src/components/Refine.tsx` (created)
- `docs/CafeHTML.md`
- `docs/log.md`
...
CSS was previously extracted from `src/app/page.tsx` to `src/app/globals.css` (3715 lines). This session applied consistency improvements for FRAME/SCENE mode-color switching.
...
**VisionScan stripped:**
- Removed `_cache`, `_inFlight`, `deduped()`, `clearCache()`, `_visionCache`
...
- Updated in: prompt-bar.js, gallery.js, src/lib/enhancer.ts, prompt-builder.js, workspace.js

**Catch-up scan in src/lib/api.ts:**
...
**Image dispatch fix in src/lib/api.ts:**
...
Gated the catch-up scan in `src/lib/api.ts` on `scanTiming === 'load'`. On Generate now correctly skips individual VisionScan and sends all images inline to the enhancer for holistic analysis.

**Change:**
- `src/lib/api.ts` — catch-up scan condition changed from `if (DescriptionRegistry)` to `if (DescriptionRegistry && SettingsContext.getScanTiming() === 'load')`
...
- **`src/components/ModulePanel.tsx`** — `syncModuleState()` now writes flat `{ html }` for noSlots sections instead of `{selected, slots}`. `_loadFromState()` accepts both old `{selected, slots}` and new `{html}` shapes (backward compat). `_resetState()` resets html directly for noSlots.
...
- **`src/lib/api.ts`** — `snapshotModuleState()` writes flat `{ html }` for STAGE/STYLE in gallery cell snapshots. `hasModuleImages()` unchanged (reads payload, not ModuleState).

**Backward compat:** `_loadFromState()` handles old saved projects and gallery cells — extracts html from `data.slots[0].html` when old shape detected. `PromptBuilder.collect()` output format unchanged — downstream (src/lib/enhancer.ts, workspace.js) unaffected.
...
**Diagnostics Added:**
- `src/lib/api.ts` logs payload image fingerprints before enhancement.
- `src/lib/api.ts` logs payload image fingerprints after On Load catch-up.
- `src/lib/api.ts` logs generation manifest fingerprints before `googleGenerate()`.
- `src/lib/enhancer.ts` logs image fingerprints, cache policy, cache key when applicable, and explicit "cache disabled" messages for inline-image runs.
...
**Task 2 — UUID stamp on gallery cells (`src/lib/api.ts`, `gallery.js`):**
...
**Task 4 — Shared `callGoogleAPI` (`src/lib/api.ts`, `refine.js`):**
...
- `callRefineAPI()` in `refine.js` updated to use `api.ts functions.callGoogleAPI`
- Exported: `api.ts functions.callGoogleAPI`

**Task 5 — Enhancer cache key fix (`src/lib/enhancer.ts`):**
...
**Task 6 — Module snapshots preserve images (`src/lib/api.ts`, `workspace.js`):**
...
**Task 7 — Description persistence (`src/lib/db.ts`, `module-panel.js`):**
...
- `ModuleContext state[sectionKey]` → `config.stateTarget[config.stateKey]` throughout
- `ModuleContext = { makeSection }` exported
...
**`src/components/Studio.tsx` — main Studio logic:**
- `StudioContext.open(config)` — entry point. `config`: `{ imgUrl, uuid, ratio, caller, onDone }`
...
- `callStudioAPI()` — assembles parts (canvas image, annotation layer if strokes present, studio module images), calls `api.ts functions.callGoogleAPI`, appends "Focus on the annotated area." when strokes present
- `close()` — saves history to `_session`, calls `onDone(_latestUrl)`

**`src/components/StudioModule.tsx` — reference panel:**
- Flat module (no slots, no eye, no link) via `ModuleContext.makeSection` factory
...
**`src/app/page.tsx` — HTML + CSS + entry points:**
...
**Files Touched:**
- `src/components/Studio.tsx` (created)
- `src/components/StudioModule.tsx` (created)
- `src/app/page.tsx`
- `docs/log.md`
...
**Add-reference card redesign (`src/components/StudioModule.tsx`, `src/app/page.tsx`):**
...
- Removed `[contenteditable="true"]:focus-visible` from global rule in `src/app/globals.css` — was drawing an orange outline outside the NAME field

**Layout shift fix (`src/app/page.tsx` inline style):**
...
**Collapsed state design (`src/app/globals.css`, `src/app/page.tsx`):**
...
- `src/app/page.tsx` (inline `<style>` + HTML structure)
- `src/app/globals.css` (focus rule removed, collapsed state added)
...
**Changes:**
- `src/app/page.tsx` — inline block removed entirely (was disabled, now deleted). `src/lib/db.ts`, `prompt-bar.js`, `module-panel.js`, `gallery.js`, `sequence-bar.js`, `refine.js` now in script tags. `src/app/globals.css` linked in `<head>` (was never loaded despite being extracted).
- `src/components/PromptBar.tsx`, `src/components/SequenceBar.tsx`, `src/app/globals.css` — added to git (were untracked despite being loaded by the page)
- `src/lib/api.ts` — `summarizePayloadImages()` diagnostic helper; fingerprint logs before enhancement, after catch-up scan, before generation
- `src/lib/enhancer.ts` — improved cache key (samples head+tail, not just length); cache disabled when any inline images present; `imageFingerprint()` helper; diagnostic logs
- `src/lib/vision.ts` — removed `_cache`/`_inFlight`/`deduped()` (caching now owned by `src/lib/db.ts`); `callGemini()` restructured with retry in `runOne(attempt)`

**Bug fixed:**
- `DescriptionRegistry.clear()` was unguarded in `src/lib/api.ts:488` — threw TypeError after every generation when Keep Descriptions was OFF. Fixed by loading `src/lib/db.ts`.

**Decisions:**
- Inline block preserved in git history (`git show HEAD~3:src/app/page.tsx`) — no need to keep it in the file
- `src/app/globals.css` link added — CSS consistency improvements from 2026-05-11 (var(--accent) fixes) were never applied to the running app
...
- `src/lib/enhancer.ts`
- `src/lib/vision.ts`
- `src/components/PromptBar.tsx` (tracked)
- `src/components/SequenceBar.tsx` (tracked)
- `src/app/globals.css` (tracked)
- `docs/log.md`
...
- `autosave()` calls `serializeModuleState(ModuleContext state)` before saving to `DB.moduleState`
...
**`src/components/ModulePanel.tsx`:**
- All `DB.images.put()` calls updated to pass `activeProjectId`
...
- All `DB.images.put()` calls pass `activeProjectId`
...
**`src/app/page.tsx`:**
- Remaining inline `<style>` block (~3950 lines) migrated to `src/app/globals.css`
- `sm-header-add` button added to studio module header (`+` to add new reference group)

**`src/app/globals.css`:**
- Studio overlay styles absorbed from src/app/page.tsx inline block
...
- `src/app/globals.css` — `.studio-module-panel .plr-name:focus` gets `border-bottom: 1px solid #c7c7c7` as a focus indicator (orange border suppressed in studio context by module-panel.js)
...
- `src/components/Studio.tsx` now stores Studio sessions per source image UUID in `DB.studioState.histories[uuid]`
...
- `New` creates `Project N` directly instead of using `prompt`
...
**`src/components/StudioModule.tsx` — full rework:**
...
**`src/app/globals.css` — Studio module visual polish:**
...
- `src/lib/api.ts` — Generate button could stay stuck spinning: it gated on the shared `_activeRequests` counter, which `generateLayerImage` also mutates. Added a dedicated `_activeGenerations` counter for the button.
...
**Dead-code removal:**
- Deleted `src/components/Refine.tsx` (`RefineArea`, ~577 lines), its `<script>` tag, and the orphaned `#refine-overlay` markup. Superseded by Studio; nothing invoked it. Shared `.refine-*` CSS classes kept (Studio reuses them).
...
- `src/lib/net.ts` (new) — `CafeNet.fetchJSON(url, options, { label, maxRetries, timeoutMs })`. The duplicated fetch + 429 retry/backoff in `src/lib/api.ts`, `src/lib/enhancer.ts`, `src/lib/vision.ts` now route through it (vision uses `timeoutMs` for its AbortController path).
...
**Files Touched:**
- `src/lib/api.ts`, `src/lib/enhancer.ts`, `src/lib/vision.ts`, `src/components/Gallery.tsx`, `src/components/ModulePanel.tsx`, `src/components/PromptBar.tsx`, `logic/prompt-builder.js`, `src/lib/workspace.ts`, `logic/settings.js`
- `src/lib/net.ts` (created), `src/components/Refine.tsx` (deleted)
- `src/app/page.tsx`, `src/app/globals.css`, `README.md`, `.gitignore`
...
**Parallel generation (`src/lib/api.ts`):**
...
**Partial-success (`src/lib/api.ts`):**
...
**NB2 thinking-level option (`settings.js`, `src/lib/api.ts`, `src/app/page.tsx`):**
...
- `src/lib/api.ts` reads `CafeSettings.getActiveThinkingLevel()` instead of the hardcoded model value. NB and Pro return null (thinkingConfig omitted). Value lowercased (`minimal`) to match the docs.
- Cost-vs-thinking display was considered and skipped — thinking tokens vary per prompt; no flat per-image figure to show.

**Files Touched:**
- `src/lib/api.ts`, `logic/settings.js`, `src/app/page.tsx`
- `docs/log.md`, `docs/CafeHTML.md`
...
Implemented the new CafeHTML Module Panel design from the S-C handoff as a vanilla JS panel in `src/components/ModulePanel.tsx` with matching CSS in `src/app/globals.css`.
...
- Preserved the existing generation bridge by syncing the new `cafeModule` state back into compatible legacy `ModuleContext state.subject/stage/style` HTML snapshots.
...
- `STUDIO` opens `StudioContext.open(...)` for the selected module image and writes the returned image back into the same module record.
...
- Removed prompt-bar global reference chip UI and hidden ref file input from `src/app/page.tsx`.
...
- `prompt-bar.js` now routes the plus button to `ModuleContext.openUpload()`.
- Upload ownership remains in `module-panel.js`; no separate global ref lane reintroduced.

**Safety + compatibility notes:**
- Kept empty shim: `refState = { FRAME: [], SCENE: [] }` and no-op `renderChips()` to avoid crashing old callers during transition.
- Added `logic/composition.js` script include in `src/app/page.tsx` so enhancer normalization resolves correctly.
